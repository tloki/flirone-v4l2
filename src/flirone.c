...
//#define FRAME_FORMAT0 V4L2_PIX_FMT_Y16
...
void startv4l2() {
    ...
    vid_format0.fmt.pix.width = FRAME_WIDTH0 * 2;
    vid_format0.fmt.pix.height = FRAME_HEIGHT0;
    vid_format0.fmt.pix.pixelformat = FRAME_FORMAT0;
    vid_format0.fmt.pix.sizeimage = framesize0 * 2;
    vid_format0.fmt.pix.field = V4L2_FIELD_NONE;
    vid_format0.fmt.pix.bytesperline = linewidth0 * 2;
    ...
    // region video2 - mixed rgb image and thermal
    //open video_device2
    printf("using output device: %s\n", video_device2);
...
// fb_proc2 is an 24bit RGB buffer
    ...
    printf("0: %u 100: %u\n", (unsigned int) pix[0], (unsigned int) pix[100]);
    write(fdwr0, &pix[0], framesize0 * 2);
...
int EPloop(unsigned char *colormap) {
    ...
    printf("Successfully find the FLIR One G2/G3 device\n");
    ...
    printf("Successfully claimed interface 0,1,2 from camera (thermal, rgb and MSX - multi spectral imaging)\n");
    ...
    while (1) {
        ...
        switch (state) {
            ...
            case 2:
                ...
                int length = 16;
                unsigned char my_string2[16] = {0xcc, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x41, 0x00, 0x00, 0x00,
                                                0xF8, 0xB3, 0xF7, 0x00};
                ...
                length = 16;
                unsigned char my_string3[16] = {0xcc, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x33, 0x00, 0x00, 0x00,
                                                0xef, 0xdb, 0xc1, 0xc1};
                printf("\nEP 0x02 to be sent Hexcode: %i Bytes[", length);
                for (i = 0; i < length; i++) {
